...
export async function middleware(req: NextRequest) {
  ...
  // 1) API로 UUID 조회 (미들웨어 내 ISR 캐시 300초)
  const apiRes = await fetch(
    `${process.env.API_BASE_URL}/short/${code}`,
    { next: { revalidate: 300 } }
  );
  if (!apiRes.ok) {
    return NextResponse.next(); // 없는 코드면 404 처리
  }
  ...
  if (isSocial) {
    // 봇/크롤러 → 내부 rewrite → OG 메타 페이지
    const url = req.nextUrl.clone();
    url.pathname = `/share/${uuid}`;
    return NextResponse.rewrite(url);
  } else {
    // 일반 브라우저 → 즉시 메인 도메인으로 redirect
...
export const config = {
  matcher: ['/:code*'], // 루트 바로 밑의 코드 경로에만 적용
};